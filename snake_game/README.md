...
* Scoreboard tracks points continuously and alerts the player upon game-ending collisions.
...
* **State Management:** `Scoreboard` Class encapsulates score tracking, screen drawing, and game-over state rendering.

## Concepts Learned & Applied

* **Classes & Constructors:** Setting initial attributes and state using `__init__()` and instantiating objects.
* **Inheritance & Polymorphism:** Extending the `Turtle` base class using `super().__init__()` to quickly and directly build UI elements.
* **Event Listening:** Binding user keyboard inputs (`screen.onkey()`) to object methods for interactive gameplay.
...
* **`snake.move()`**: Updates the internal $(x, y)$ coordinates of all body segments in sequence.
* **`screen.update()`**: Manually triggers a complete screen redraw, displaying the entire newly rendered frame instantly.
* **`time.sleep(0.2)`**: Delays execution between iterations (~5 FPS frame rate) to give human players time to react.
